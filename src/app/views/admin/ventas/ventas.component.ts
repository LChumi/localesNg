...
export default class VentasComponent implements OnInit{
  ...
  listarEntreFechas(){
    if(this.validarFechas()){
      this.reporteService.ventasEntreFecha(this.fechaInicio, this.fechaFin).subscribe(
        data=> {
          this.actualizarMovimientos(data);
        }
      )
  ...
  actualizarMovimientos(venta: Venta[]){
    this.listaVenta=venta;
    this.filterVenta=venta;
  }
